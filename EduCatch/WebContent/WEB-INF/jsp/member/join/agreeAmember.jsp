...
function agreeCommit(){
	console.log("약관동의함수 진입");
	var chk1 = document.agreement.agree1.checked;
	var chk2 = document.agreement.agree2.checked;
	if(!chk1){
		alert("약관1에 동의해주세요");
		return false;
	}
		...
		$("#agreement").attr("action","amemberJoinForm.ec");
		$("#agreement").submit();
	}
...
<body>
	...
	<p class="title-p" align="right">EduCatch 회원으로 가입하기 위해서는 이용약관, 개인정보처리방침에 동의하셔야 합니다.</p>
...
<div align="left" style="height: 170px; padding:0 20px 20px 20px; overflow-y : auto; width: 70%; box-sizing: border-box;border: 1px solid #ccc;margin: 10px; line-height: 20px;" tabindex="0">
		...
		<br>제 1 조(목적)<br><br>
		본 약관은 EduCatch가 제공하는 모든 서비스(이하 "서비스")의 이용조건 및 절차, 회원과 EduCatch의 권리, 의무, 책임사항과 기타 필요한 사항을 규정함을 목적으로 합니다.<br>

		<br>제 2 조(약관의 효력과 변경)<br><br>
		1. EduCatch은 이용자가 본 약관 내용에 동의하는 경우, EduCatch의 서비스 제공 행위 및 회원의 서비스 사용 행위에 본 약관이 우선적으로 적용됩니다.<br>
		2. EduCatch은 약관을 개정할 경우, 적용일자 및 개정사유를 명시하여 현행약관과 함께 EduCatch의 초기화면에 그 적용일 7일 이전부터 적용 전일까지 공지합니다. 단, 회원에 불리하게 약관내용을 변경하는 경우에는 최소한 30일 이상의 사전 유예기간을 두고 공지합니다. 이 경우 국가공간정보포털은 개정 전 내용과 개정 후 내용을 명확하게 비교하여 회원이 알기 쉽도록 표시합니다.<br>
		3. 변경된 약관은 EduCatch 홈페이지에 공지하거나 e-mail을 통해 회원에게 공지하며, 약관의 부칙에 명시된 날부터 그 효력이 발생됩니다. 회원이 변경된 약관에 동의하지 않는 경우, 회원은 본인의 회원등록을 취소(회원탈퇴)할 수 있으며, 변경된 약관의 효력 발생일로부터 7일 이내에 거부의사를 표시하지 아니하고 서비스를 계속 사용할 경우는 약관 변경에 대한 동의로 간주됩니다.<br>
